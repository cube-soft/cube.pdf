...
namespace Cube.Pdf.Pinstaller
{
    ...
    public class Installer
    {
        ...
        private void Uninstall(DeviceConfig src, SpoolerService service)
        {
            ...
            var monitors = cp.PortMonitors.Convert().OfType<IInstallable>().ToList();
            var ports    = cp.Ports.Convert().OfType<IInstallable>().ToList();
            var drivers  = cp.PrinterDrivers.Convert().OfType<IInstallable>().ToList();
            var printers = cp.Printers.Convert().OfType<IInstallable>().ToList();
